...
import com.mordeccai.quickcam.contracts.LifeCycleContract
import com.mordeccai.quickcam.data.AppDatabase
import java.util.*

open class BasePresenter(val context: Context): BaseObservable(), LifeCycleContract {
    ...
    fun alert(title: String ="Alert", body: String ="", icon: Int = 0, isMaterial: Boolean = false, onOk: () -> Unit = {}){
    ...
    fun confirm(title: String ="Confirm", body: String ="", onOk: (isChecked: Boolean) -> Unit, icon: Int = 0, checkBoxText: CharSequence="",  hasCheckBox: Boolean= false ,isMaterial: Boolean = false){
        ...
        parent.layoutParams = LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT)
